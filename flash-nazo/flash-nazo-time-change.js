...
var answer = [] // ユーザーの回答
var setSecond = 0
var spendSecond = 0 // 使った時間
var time_easy = time_easy_special || 60 // easyの時間
var time_normal = time_normal_special || 30 // normalの時間
var time_hard = time_hard_special || 15 // hardの時間
var time_master = time_master_special || 5 //masterの時間
...
$(function() {
    ...
    $(".readme ul").append("<li>一定の時間で次の問題に切り替わります</li>")
    $(".readme ul").append("<li>「次へ」を押しても次へ進めます</li>")
    $(".readme ul").append("<li>回答は答えに近いものを選ぶ方式です</li>")
    $(".readme ul").append("<li>この画面を押すとゲームスタートです</li>")

    // 秒数の表示変更
    $(".time_easy").text(time_easy + "秒");
    $(".time_normal").text(time_normal + "秒");
    $(".time_hard").text(time_hard + "秒");
    $(".time_master").text(time_master + "秒");
    ...
    $(".choice-level-content").on({
      "click": function() {
        switch($('.choice-level-content').index(this)) {
          case 0:
            setSecond = time_easy
            level = "EASY"
            break;
          case 1:
            setSecond = time_normal
            level = "NORMAL"
            break;
          case 2:
            setSecond = time_hard
...
// 次へが押された時の処理
function nextButton() {
  spendSecond = spendSecond + second;
  clearInterval(timerId);
  counter = counter + 1
  gameStart();
}
